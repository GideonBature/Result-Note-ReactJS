            ...
            ></i>
                ...
                <div className='flex-1 mt-5 space-y-2 duration-700'>
                    ...
                    {navLinks.map((navMenuLink, index) => (
                        <li key={index} className={`group flex items-center hover:bg-cyan-600 rounded-xl p-2 ${!open ? 'hover:bg-cyan-600 pl-4 -ml-3 -mr-2.5 rounded-xl duration-700' : 'hover:bg-cyan-600 pl-4 -ml-3 -mr-2.5 rounded-xl'}`} >
                            <Link to={navMenuLink.path} className="flex items-center">
                                ...
                                {!open ? (
                                    <span className={`tooltip ml-7 cursor-default text-black text-xs font-semibold bg-cyan-500 rounded p-1 px-2 opacity-0 invisible scale-0 transition-transform transform group-hover:opacity-100 group-hover:visible group-hover:scale-100`} onClick={(e) => e.preventDefault()}>{navMenuLink.text}</span>
                                ) : null}
                            </Link>
                        </li>
                    ))}
