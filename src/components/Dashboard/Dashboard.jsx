                ...
                <ul className="flex-1 mt-8 space-y-8">
                    ...
                    <li className="relative font-normal text-sm mb-4 -ml-3 -mr-2">
                        ...
                        <input
                            ...
                            className={`bg-sky-900 p-2 placeholder:pl-8 w-full rounded-xl px-2 ${!open ? 'w-full p-2 pr-4 right-0' : ''}`}
                        />
                    </li>
                    {/* Map through navLinks objs & render dynamically */}
                    {navLinks.map((link, index) => (
                        <li key={index} className={`flex items-center hover:bg-cyan-600 w-full rounded-xl p-2 px-1 ${!open ? 'hover:bg-cyan-600 w-full pl-4 -ml-3 rounded-xl' : ''}`}>
